...
class StudentTest extends Controller
{
    ...
    function get($KeyWord = "" , $W = [] , $P = 1 , $N = 10){
        try{
            ...
            $T = Db::table('cnes_paper_bank')->alias('pr')->where(['TypeID'=>1])->count();
            //数据查询
            $L = Db::table('cnes_paper_bank')->alias('pr')
                ->join('cnes_user u','u.UserID=pr.CUID')
                ->page($P,$N)
                ->where(['TypeID'=>1])
                ->field('u.Name as UserName , pr.PName , pr.TotalScore , pr.PIID')
                ->select();
                ...
                case 1:
                    ...
                    if(isset($Answer[$Key]) && is_array($Answer[$Key])){
                        foreach ($Answer[$Key] as $key => $value){
                            $MyAnswer .= StudentTest::$NumLetterM[$value - 1];
                        }
                    }
